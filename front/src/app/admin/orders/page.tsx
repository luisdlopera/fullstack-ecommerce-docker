...
import { ordersApi } from '@/services/admin-api';
  ...
  return (
    <>
      ...
      <div className='mb-4 flex flex-wrap items-center gap-3'>
        <SearchInput value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder='Buscar por ID, email o nombre...' />
        <FilterSelect value={statusFilter} onChange={(v) => { setStatusFilter(v); setPage(1); }} options={ORDER_STATUS_OPTIONS} placeholder='Todos los estados' />
        <FilterSelect value={paymentFilter} onChange={(v) => { setPaymentFilter(v); setPage(1); }} options={PAYMENT_STATUS_OPTIONS} placeholder='Estado de pago' />
      </div>
      ...
      >
        ...
        <div className='space-y-6 p-6'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div><span className='text-xs font-medium text-gray-500'>Estado</span><div className='mt-1'><StatusBadge value={order.status} type='order' /></div></div>
            <div><span className='text-xs font-medium text-gray-500'>Pago</span><div className='mt-1'><StatusBadge value={order.paymentStatus} type='payment' /></div></div>
          </div>
