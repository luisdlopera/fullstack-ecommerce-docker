...
import type { AdminOrder, TopProduct } from '@/types/admin';
      ...
      {isLoading ? (
        ...
        <>
          <div className='mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4'>
            <StatCard title='Ventas totales' value={formatCurrency(summary.totalSales)} icon={DollarSign} description='Acumulado' />
            <StatCard title='Órdenes totales' value={summary.totalOrders.toLocaleString()} icon={ShoppingCart} />
            <StatCard title='Usuarios' value={summary.totalUsers.toLocaleString()} icon={Users} />
            <StatCard title='Productos activos' value={summary.activeProducts.toLocaleString()} icon={Package} />
            <StatCard title='Ticket promedio' value={formatCurrency(summary.avgTicket)} icon={TrendingUp} />
            <StatCard title='Órdenes pendientes' value={summary.pendingOrders.toLocaleString()} icon={Clock} />
            <StatCard title={`Ingresos (${PERIOD_OPTIONS.find((o) => o.value === period)?.label ?? period})`} value={formatCurrency(summary.periodRevenue)} icon={BarChart3} description={`${summary.periodOrders} órdenes`} />
            <StatCard title='Sin stock' value={summary.outOfStock.toLocaleString()} icon={AlertTriangle} description='Productos agotados' />
          </div>
          ...
          {products.map((item, i) => (
            <div key={item.product?.id ?? i} className='flex items-center gap-4 px-6 py-3'>
              ...
              <div className='flex-1'>
                <p className='text-sm font-medium text-gray-900'>{item.product?.title ?? 'Producto eliminado'}</p>
