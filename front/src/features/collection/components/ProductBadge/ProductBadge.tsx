...
export function ProductBadge({ kind, discountPercent = 0 }: ProductBadgeProps) {
	if (kind === 'new') {
		return (
			<span className='rounded-md bg-neutral-900 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-white uppercase'>
				Nuevo
			</span>
		);
	}
	if (kind === 'soldOut') {
		return (
			<span className='rounded-md bg-red-600 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-white uppercase'>
				Agotado
			</span>
		);
	}
