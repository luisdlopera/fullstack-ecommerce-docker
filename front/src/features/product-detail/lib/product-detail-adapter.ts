...
import type { ProductDetail, ProductBenefit, SimilarProduct } from '../types';
...
export function mapApiProductToProductDetail(product: Product): ProductDetail {
	const images =
		product.ProductImage.length > 0 ? product.ProductImage.map((img) => img.url) : ['/img/shirt/shirt-black-1.png'];
